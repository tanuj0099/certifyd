...
function Breadcrumb({ items }) {
  return (
    <nav aria-label="Breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
      <Link href="/" style={{ color: 'var(--text-4)', textDecoration: 'none', fontFamily: FM, fontSize: '11px', letterSpacing: '0.08em' }}>
        Home
      </Link>
      {items.map((item, i) => (
        <span key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          ...
          {item.href ? (
            <Link href={item.href} style={{ color: 'var(--text-3)', textDecoration: 'none', fontFamily: FM, fontSize: '11px', letterSpacing: '0.08em', transition: 'color 150ms' }}>
              {item.label}
            </Link>
          ) : (
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: FS, paddingTop: isMobile ? '112px' : '128px' }}>
      ...
      <div style={{
        ...
        {/* Global DynamicBreadcrumb handles navigation */}
      </div>
