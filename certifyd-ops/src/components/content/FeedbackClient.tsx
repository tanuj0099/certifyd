...
export function FeedbackClient({ initialRecords, userRole }: FeedbackClientProps) {
  ...
  const [toolFilter, setToolFilter] = useUrlFilter<string>('tool', 'ALL');
  const [ratingFilter, setRatingFilter] = useUrlFilter<string>('rating', 'ALL');
  const [sentimentFilter, setSentimentFilter] = useUrlFilter<string>('sentiment', 'ALL');
  ...
  async function handleStatusChange(id: string, newStatus: string) {
    setLoading(true);
    try {
      await updateContentStatusAction('feedback_reviews', id, newStatus);
  ...
  async function handleAddNote(id: string) {
    ...
    setLoading(true);
    try {
      const res = await addSubmissionNoteAction('feedback_reviews', id, newNoteText.trim());
