          ...
          >
            <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto text-center md:text-left">
              ...
              <button 
                ...
                className={`font-semibold text-sm md:text-base px-3 py-1.5 rounded-lg border max-w-[200px] truncate transition-all cursor-pointer ${activeSlot === 'A' ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-[color:var(--bg-alt)] dark:bg-slate-800 border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500 text-[color:var(--text)] dark:text-slate-100'}`}
                title="Click to change Cert 1"
              >
                {activeCompareCertA.name}
              </button>
              <span className="text-[color:var(--text-4)] hidden md:block">vs</span>
              <button 
                onClick={() => setActiveSlot('B')}
                className={`font-semibold text-sm md:text-base px-3 py-1.5 rounded-lg border max-w-[200px] truncate transition-all cursor-pointer ${activeSlot === 'B' ? 'bg-blue-600 border-blue-600 text-white shadow-lg' : 'bg-[color:var(--bg-alt)] dark:bg-slate-800 border-slate-300 dark:border-slate-700 hover:border-slate-400 dark:hover:border-slate-500'} ${compareCertB && activeSlot !== 'B' ? 'border-solid text-[color:var(--text)] dark:text-slate-100' : (!compareCertB && activeSlot !== 'B' ? 'border-dashed text-[color:var(--text-4)] dark:text-[color:var(--text-4)]' : '')}`}
