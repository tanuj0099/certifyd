...
export default async function DashboardPage() {
  ...
  try {
    const results = await Promise.allSettled([
      ...
      supabaseAdmin.from('feedback_reviews').select('id, created_at, rating, tool_used, user_id').order('created_at', { ascending: false }).limit(10),
  ...
  return <DashboardClient initialStats={JSON.parse(JSON.stringify(initialStats))} initialActivities={JSON.parse(JSON.stringify(initialActivities))} />;
}
